...
from momentum.services.core.factory.handler import MomentumServiceHandlerFactory
...
def generateServiceMappings(svc_cfg, registry_path=forms.DEFAULT_REGISTRY_PATH):
	...
	for path, service in services:
		...
		new_mapping = MomentumServiceHandlerFactory.default(service).mapping(path)
			...
			if config.debug:
				...
				if services_cfg.get('hooks', {}).get('profiler', {}).get('enabled', False) == True:
					try:
						...
						def profile_run(app):
							filepath = services_cfg['hooks']['apptrace'].get('filename', "var/export/performance/FatCatMap-Services.profile")
